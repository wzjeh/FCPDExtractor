...
前提条件：
papers 文件夹中的 PDF 应已按数字命名（1.pdf, 2.pdf, ...）
"""
...
def main():
    """
    主函数：
    批量处理 papers 文件夹中的所有 PDF
    为每个 PDF 生成两个独立文件：
    - {number}_reaction.json：reaction_summary
    - {number}_impact.txt：impact_analysis
    
    前提：papers 文件夹中的 PDF 已按数字命名（1.pdf, 2.pdf, ...）
    """
    print("=" * 60)
    print("微调数据集生成工具")
    print("=" * 60)
    ...
    print("\n初始化 qwen-plus 模型...")
    try:
        llm = QwenLLM(api_key_env_var="QWEN_API_KEY", model_name="qwen-plus")
        print("✓ 模型初始化成功")
    except Exception as e:
        print(f"✗ 模型初始化失败: {e}")
