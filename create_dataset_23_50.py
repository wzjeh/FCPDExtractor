#!/usr/bin/env python3
"""
微调数据集生成脚本 - 处理第23-50号PDF文件
使用 qwen-plus-2025-09-11 模型

功能：
使用 qwen-plus-2025-09-11 模型直接生成：
...
前提条件：
papers 文件夹中的 PDF 应已按数字命名（23.pdf, 24.pdf, ..., 50.pdf）
"""
...
def main():
    """
    主函数：
    批量处理 papers 文件夹中的第23-50号 PDF
    为每个 PDF 生成两个独立文件：
    - {number}_reaction.json：reaction_summary
    - {number}_impact.txt：impact_analysis
    
    前提：papers 文件夹中的 PDF 已按数字命名（23.pdf, 24.pdf, ..., 50.pdf）
    """
    print("=" * 60)
    print("微调数据集生成工具 - 处理第23-50号PDF")
    print("使用模型: qwen-plus-2025-09-11")
    print("=" * 60)
    ...
    for i in range(23, 51):  # 23到50
        pdf_path = os.path.join(papers_dir, f"{i}.pdf")
        if os.path.exists(pdf_path):
            pdf_files.append(pdf_path)
    
    if not pdf_files:
        print(f"\n未在 {papers_dir} 中找到第23-50号 PDF 文件")
        return
    
    # 按数字排序
    pdf_files = sorted(pdf_files, key=lambda x: int(os.path.splitext(os.path.basename(x))[0]))
    print(f"\n找到 {len(pdf_files)} 个 PDF 文件 (第23-50号)")
    
    # 步骤2: 初始化qwen-plus-2025-09-11模型
    print("\n初始化 qwen-plus-2025-09-11 模型...")
    try:
        llm = QwenLLM(api_key_env_var="QWEN_API_KEY", model_name="qwen-plus-2025-09-11")
        print("✓ 模型初始化成功")
    except Exception as e:
        print(f"✗ 模型初始化失败: {e}")
